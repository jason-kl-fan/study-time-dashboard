...
function renderDebugStats(currentRecords, range, compareMode) {
  ...
  rows.forEach(({ person, records: personRecords, totals, totalMinutes }) => {
    ...
    lines.push(`- raw person values: ${personRecords.map((record) => JSON.stringify(record.person)).join(', ') || '(none)'}`);
    lines.push(`- raw category values: ${personRecords.map((record) => JSON.stringify(record.category)).join(', ') || '(none)'}`);
    lines.push('');
  });

  debugStatsBox.textContent = lines.join('\n');
}
